    ...
    match hex::decode(challenge_seed) {
        Ok(challenge_seed_bytes) => {
            ...
            match read_vrf_key(pool_vrf_skey) {
                Ok(vrf_skey) => {
                    if vrf_skey.key_type != "VrfSigningKey_OptimumVRF" {
    ...
    match hex::decode(challenge_seed) {
        Ok(challenge_seed_bytes) => {
            ...
            match read_vrf_key(pool_vrf_vkey) {
                Ok(vrf_vkey) => {
                    if vrf_vkey.key_type != "VrfVerificationKey_OptimumVRF" {
                        handle_error("Pool VRF Vkey must be of type: VrfVerificationKey_OptimumVRF");
                        return;
                    }
