...
use bcc_shardagnostic_network::protocols::chainsync::Listener;
use bcc_shardagnostic_network::BlockHeader;
...
impl PoolToolNotifier {
    pub fn send_to_pooltool(&mut self, header: &BlockHeader) {
        if self.last_node_version_time.elapsed() > Duration::from_secs(3600) {
            ...
            match Command::new(&self.bcc_node_path)
                .arg("--version")
                .stdin(Stdio::null())
                .stdout(Stdio::piped())
                .output()
            {
                Ok(output) => {
                    ...
                    let cap = Regex::new("bcc-node (\\d+\\.\\d+\\.\\d+) .*\ngit rev ([a-f0-9]{5}).*")
                        .unwrap()
                    ...
                    info!("Checking bcc-node version: {}", &self.node_version);
                    self.last_node_version_time = Instant::now();
                }
                Err(err) => {
                    panic!("Error getting bcc-node version: {}", err)
                }
            }
        }
