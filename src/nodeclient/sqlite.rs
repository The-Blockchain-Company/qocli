...
use bcc_shardagnostic_network::{BlockHeader, BlockStore};
        ...
        let tx = db.transaction()?;
        {
            ...
            if version < 1 {
                ...
                tx.execute(
                    ...
                    protocol_sentry_version INTEGER NOT NULL, \
                ...
                match db.query_row(
                    ...
                    Err(_) => {
                        match network_magic {
                            ...
                            5 => {
                                // aurum-blue genesis hash
                                info!("Start nonce calculation for aurum-blue testnet.");
                                String::from("60ba98183b381c933acaa298a815e090bdb86726fd19562e12f6ed6aa78caef2")
                            }
                            7 => {
                                // aurum-white genesis hash
                                info!("Start nonce calculation for aurum-white testnet.");
                                String::from("b72001cddc21713dd63d899c1993a5b0728cd909eb261fff0e50d10f46340f1f")
                            }
                            8 => {
                                // aurum-purple genesis hash
                                info!("Start nonce calculation for aurum-purple testnet.");
        ...
        let tx = db.transaction()?;
        {
            ...
            for block in pending_blocks.drain(..) {
                ...
                insert_stmt.execute_named(named_params! {
                    ...
                    ":protocol_sentry_version" : block.protocol_sentry_version,
                })?;
            }
        }
