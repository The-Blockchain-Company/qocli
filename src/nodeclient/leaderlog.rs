...
fn read_byron_genesis(byron_genesis: &Path) -> Result<ColeGenesis, Error> {
    let buf = BufReader::new(File::open(byron_genesis)?);
    Ok(serde_json::from_reader(buf)?)
}

fn read_sophie_genesis(sophie_genesis: &Path) -> Result<SophieGenesis, Error> {
    let buf = BufReader::new(File::open(sophie_genesis)?);
    Ok(serde_json::from_reader(buf)?)
}
...
fn get_sophie_transition_epoch(network_magic: u32) -> i64 {
    match network_magic {
        764824073 => {
            // mainnet
            208
...
fn get_first_slot_of_epoch(byron: &ColeGenesis, sophie: &SophieGenesis, current_slot: i64) -> (i64, i64) {
    let sophie_transition_epoch = get_sophie_transition_epoch(sophie.network_magic);
    let byron_epoch_length = 10 * byron.protocol_consts.k;
    let byron_slots = byron_epoch_length * sophie_transition_epoch;
    let sophie_slots = current_slot - byron_slots;
    let sophie_slot_in_epoch = sophie_slots % sophie.epoch_length;
    let first_slot_of_epoch = current_slot - sophie_slot_in_epoch;
    let epoch = (sophie_slots / sophie.epoch_length) + sophie_transition_epoch;

    (epoch, first_slot_of_epoch)
}

fn slot_to_naivedatetime(byron: &ColeGenesis, sophie: &SophieGenesis, slot: i64) -> NaiveDateTime {
    let sophie_transition_epoch = get_sophie_transition_epoch(sophie.network_magic);
    let network_start_time = NaiveDateTime::from_timestamp(byron.start_time, 0);
    let byron_epoch_length = 10 * byron.protocol_consts.k;
    let byron_slots = byron_epoch_length * sophie_transition_epoch;
    let sophie_slots = slot - byron_slots;

    let byron_secs = (byron.block_version_data.slot_duration * byron_slots) / 1000;
    let sophie_secs = sophie_slots * sophie.slot_length;

    network_start_time + Duration::seconds(byron_secs) + Duration::seconds(sophie_secs)
}

fn slot_to_timestamp(byron: &ColeGenesis, sophie: &SophieGenesis, slot: i64, tz: &Tz) -> String {
    let slot_time = slot_to_naivedatetime(byron, sophie, slot);
...
pub(crate) fn calculate_leader_logs(
    ...
    if !sophie_genesis.exists() {
        handle_error(format!(
            "Invalid Path: --sophie-genesis {}",
            sophie_genesis.to_string_lossy()
        ));
        return;
    }
    ...
    match read_byron_genesis(byron_genesis) {
        Ok(byron) => {
            debug!("{:?}", byron);
            match read_sophie_genesis(sophie_genesis) {
                Ok(sophie) => {
                    debug!("{:?}", sophie);
                    ...
                    let tip_time = slot_to_naivedatetime(&byron, &sophie, tip_slot_number).timestamp();
                    ...
                    let additional_slots: i64 = match ledger_set {
                        LedgerSet::Mark => sophie.epoch_length,
                        LedgerSet::Set => 0,
                        LedgerSet::Go => -sophie.epoch_length,
                    };
                    let (epoch, first_slot_of_epoch) =
                        get_first_slot_of_epoch(&byron, &sophie, tip_slot_number + additional_slots);
                    debug!("epoch: {}", epoch);
                    let first_slot_of_prev_epoch = first_slot_of_epoch - sophie.epoch_length;
                    ...
                    let stability_window: i64 =
                        ((3 * byron.protocol_consts.k) as f64 / sophie.active_slots_coeff).ceil() as i64;
                                    ...
                                    match get_prev_hash_before_slot(&db, first_slot_of_prev_epoch) {
                                        Ok(nh) => {
                                            ...
                                            match read_vrf_key(pool_vrf_skey_path) {
                                                Ok(pool_vrf_skey) => {
                                                    if pool_vrf_skey.key_type != "VrfSigningKey_OptimumVRF" {
                                                        handle_error("Pool VRF Skey must be of type: VrfSigningKey_OptimumVRF");
                                                        return;
                                                    }
                                                    ...
                                                    let epoch_slots_ideal = (sigma.to_f64().unwrap()
                                                        * (sophie.epoch_length.to_f64().unwrap()
                                                        * sophie.active_slots_coeff)
                                                        * (1.0 - d)
                                                        * 100.0)
                                                        .round()
                                                        / 100.0;
                                                    ...
                                                    let c: BigDecimal = ln(&(BigDecimal::one()
                                                        - BigDecimal::from_f64(sophie.active_slots_coeff).unwrap()));

                                                    // Calculate all of our assigned slots in the epoch (in parallel)
                                                    let assigned_slots = (0..sophie.epoch_length)
                                                    ...
                                                    for (i, slot) in sorted(assigned_slots.iter()).enumerate() {
                                                        ...
                                                        let slot = Slot {
                                                            ...
                                                            at: slot_to_timestamp(&byron, &sophie, *slot, &tz),
                                                        };
...
pub(crate) fn status(db_path: &Path, byron_genesis: &Path, sophie_genesis: &Path) {
    if !db_path.exists() {
                ...
                Ok(sophie) => {
                    ...
                    match get_tip_slot_number(&db) {
                        Ok(tip_slot_number) => {
                            ...
                            let system_time = Utc::now().timestamp();
                            if system_time - tip_time < 120 {
                                let (epoch, _) = get_first_slot_of_epoch(&byron, &sophie, tip_slot_number);
                                debug!("epoch: {}", epoch);
