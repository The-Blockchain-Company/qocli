...
use qocli::nodeclient::math::{ceiling, exp, find_e, ln, round, split_ln};
use qocli::nodeclient::ping;
...
#[test]
fn test_ping() {
    let host = "north-america.relays-new.bcc-testnet.blockchain-company.io".to_string();
    let port = 3001;
...
fn test_ping_failure_address() {
    let host = "murrika.relays-new.bcc-testnet.blockchain-company.io".to_string();
    let port = 3001;
    let network_magic = 1097911063;
    let mut stdout: Vec<u8> = Vec::new();
